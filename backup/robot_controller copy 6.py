...
class Controller():
    ...
    def _update_controls(self, msg, dist_min, dist_max):
        """if self.leftfront_dist < dist_min or self.front_dist < dist_min or\
            ...
            msg.linear.x = 0
        elif self.leftfront_dist < dist_max or self.front_dist < dist_max or\
                self.rightfront_dist < dist_max:
        ...
        elif self.right_dist < dist_min or self.rightfront_dist < dist_max:
            self.robot_state += "turning slow -"
            msg.angular.z = self.speed_angular_slow
        elif self.front_dist > dist_min+dist_max and\
                self.rightfront_dist > dist_min+dist_max and\
                self.right_dist > dist_min+dist_max:
            self.robot_state += "turning fast +"
            msg.angular.z = -self.speed_angular_fast
        else:
            self.robot_state += "turning slow +"
    ...
    def follow_wall(self):
        ...
        if self.searching_wall:
            self._update_search(msg, d2)
        else:
            self._update_controls(msg, d1, d2)
